...
class Api {
  constructor() {
    ...
    if (config.cloudflaredTunnelToken !== undefined && config.cloudflareListenPort !== config.listenPort) {
      this.cloudflareServer = http.createServer(app.callback())
    }
  }
  ...
  async init() {
    ...
    if (this.cloudflareServer !== undefined) {
      await new Promise(resolve => this.cloudflareServer.listen(config.cloudflareListenPort, '127.0.0.1', resolve))
      logger.log({level: 'info', msg: `API | Initialized and reachable on http://127.0.0.1:${config.cloudflareListenPort}`})
    }
  }
  ...
  async onUpdateUser(ctx) {
    ...
    if (body.disabledWalletsByFingerprint !== undefined) {
      if (ctx.user.settings === undefined) {
        ctx.user.settings = {}
      }
      ctx.user.settings.disabledWalletsByFingerprint = body.disabledWalletsByFingerprint
      updated = true
    }
    if (updated) {
      await ctx.user.save()
