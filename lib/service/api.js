...
class Api {
  constructor() {
    ...
    router.patch('/api/satellite', rateLimitMiddleware({ threshold: 16, ttl: 60 }), satelliteAuthMiddleware, this.onNewSatelliteStats.bind(this));
  ...
  async onNewSatelliteStats(ctx) {
    ...
    validServices
      ...
      .forEach(service => {
        if (!body[service]) {
          updateQuery[`services.${service}.stats`] = null;
        } else {
          updateQuery[`services.${service}.stats`] = body[service];
        }
