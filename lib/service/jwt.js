...
class Jwt {
  ...
  async makeAccessToken({ user }) {
    return this.sign({ payload: { userId: user.id.toString() }, audience: AUDIENCE.accessToken, expiresIn: '10m' });
  }

  async makeRefreshToken({ user }) {
    return this.sign({ payload: { userId: user.id.toString() }, audience: AUDIENCE.refreshToken, expiresIn: '7d' });
  }
