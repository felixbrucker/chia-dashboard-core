    ...
    switch (body.grant_type) {
      case 'discord_auth_code':
        ...
        try {
          ...
          if (err.response && err.response.data && err.response.data.retry_after) {
            logger.log({level: 'info', msg: `OAuth | Discord | Rate limiting ourselves for ${err.response.data.retry_after} seconds`})
            this.isDiscordBeingRateLimited = true
            setTimeout(() => this.isDiscordBeingRateLimited = false, err.response.data.retry_after * 1000)
          } else {
